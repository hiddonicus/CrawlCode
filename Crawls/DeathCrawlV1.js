...
function DeathCrawlV1() {
    ...
    if (death == "STUPID") {
        ...
        temp = temp * 100;
        alert("Write " + temp + " words to choose.");
    }

    alert("The three objects are a vial colored green, a dagger, and a scroll.");
    var threeObjects = prompt("Which object do you choose? The GREEN VIAL, the DAGGER, or the SCROLL?").toUpperCase();
    exit(threeObjects);
    while (!advance) {
        if (threeObjects == "GREEN VIAL") {
            alert("Word war with someone for fifteen minutes.");
            var tempT = prompt("How many words did you write?");
            words += tempT;
            var temp = prompt("Did you win? YES or NO");
            exit(temp);
            if (temp == "YES") {
                alert("Congratulations!");
                advance = true;
            } else {
                alert("Write 100 more words.");
                advance = true;
            }
        } else if (threeObjects == "DAGGER") {
            alert("Write 750 words.");
            word += 750;
            advance = true;
        } else if (threeObjects == "SCROLL") {
            alert("Do three five minute wordwards.");
            temp = prompt("How many words did you write?");
            exit(temp);
            word += temp;
            advance = true;
        } else {
            alert("Unknown answer. Try again?");
        }
    }
    alert("You gallop on, while writing 500 words.");
    word += 500;
    alert("Suddenly you find yourself in a dark cave, with your horse gone. Around you, bluemist drifts like fog. As you stumble to your feet, a glowing blue man appears in front of you. \"So,\" he ays, \"They sent someone to test me.\" He snaps his fingers and you find yourself in an arena.");
    if (death == "STUPID") {
        alert("Write 300 words and participate in a 30 minute wordwar.");
        var temp = prompt("How many words did you write in the word war?");
        exit(temp);
        word += temp;
    } else if (death == "AVERAGE") {
        alert("Write 1000 words and participate in a ten minute wordwar to win.");
        var temp = prompt("How many words did you write in the wordwar?");
        word += temp;
    } else {
        alert("Write 10,000 words. Do ten five headed hydras. Do a word for 20 minutes against someone with equal writing speed. Regardless of the outcome, write 5000 words. You have ten minutes to write 1,111 words. Regardless of the outcome, write 7,000 words.");
        var temp = prompt("How much did you write?");
        exit(temp);
        words += temp;
    }

    var count = word + words;
    alert("Write " + count * .10 + " words as you nurse your wounds.");
    alert("You start heading back to the field, as you have finished. Write 500 words as you do this. You reach where it should be, only to realize that it's been replaced by a city.");
    count += 500;
    if (death == "STUPID") {
        alert("Write 900 words as you walk into the city and are greeted by a long lost relative.");
        count += 900;
    } else if (death == "AVERAGE") {
        alert("You write 1000 words as you walk into the city and are greeted by a long lost relative.");
        count += 1000;
    } else {
        alert("Write " + count * .10 + " words as you are greeted by a long lost relative.");
        count += count * .10;
    }
    alert("Congratulations, you've competed the crawl with " + count + " words.");
    alert("This crawl was written by HiddenStorys, as was the code for it.");
}
